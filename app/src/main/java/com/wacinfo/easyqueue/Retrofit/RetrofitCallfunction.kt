...
import com.wacinfo.queuedemo_kotlin.Retrofit.RetrofitData
import com.wacinfo.queuedemo_kotlin.Retrofit.SendQueueApi
...
class RetrofitCallfunction {
    fun requestQueue(
        activity: Activity, type: String, typecode: String, telnum: String,name:String,
...
//        dialogupload!!.show()
        ...
        var bodyName =
            name.toRequestBody("application/x-www-form-urlencoded".toMediaTypeOrNull())

        val api: SendQueueApi = retrofit.create<SendQueueApi>(SendQueueApi::class.java)

        val call: Call<RetrofitData> = api.typePost(bodyType, bodyTypeCode, bodyTel, bodyName, ID)
